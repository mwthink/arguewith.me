// Name values from https://github.com/moby/moby/blob/master/pkg/namesgenerator/names-generator.go
...
export const generatePetName = (): string => {
  const leftName = leftValues[randomIntInRange(0, leftValues.length + 1)];
  const rightName = rightValues[randomIntInRange(0, rightValues.length + 1)];
  return [leftName, rightName].join('-');
}
